...
def split_sent(sentence: str,risk: list):
    ...
    out[-1] = list(out[-1] + tmp)+risk
    
    # out = [sample(out[i],len(out[i])) for i in range(len(out))] # random text

    return out

def preprocess(h_risk:str,l_risk:str,qa_file: str, risk_file: str):
    with open(h_risk, "r", encoding = "utf-8") as H_R,\
            open(l_risk, "r", encoding = "utf-8") as L_R:
        high_risk = []
        low_risk = []
    ...
    with open(qa_file, "r", encoding = "utf-8") as f_QA,\
        ...
        for i, line in enumerate(csv.reader(f_Risk)):
            ...
            if int(line[3]) == 0:
                article.append(split_sent(text,low_risk))
            elif int(line[3]) == 1:
                article.append(split_sent(text,high_risk))
...
class all_dataset(Dataset):
    def __init__(
        ...
        # w2id = {"[PAD]": 0}
        article, risk, qa = preprocess(high_risk,low_risk,qa_file, risk_file)
